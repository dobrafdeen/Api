...
define('CHANNEL_USERNAME', '@mahdyvip0');
...
if (isset($update['callback_query'])) {
    ...
    if ($data == 'subscribe') {
        if (!is_member($user_id)) {
            send_message($user_id, "🚫 عليك الاشتراك أولاً في القناة ".CHANNEL_USERNAME);
        } else {
            // إضافة طلب اشتراك
            $subs = load_subscribers();
...
// شغّل هذا الجزء عبر كرون Vercel أو زر يدوي كل يوم (مثلاً عبر https://vercel.com/docs/cron-jobs)
if (isset($_GET['cron']) && $_GET['cron'] == '1') {
    ...
    foreach($subs as $id=>$info){
        ...
        if($info['status']=='active' && $info['end']<time()){
            send_message($id, "❌ انتهى اشتراكك وتم إلغاء عضويتك تلقائياً.");
